...
class PathToTrack {
  ...
  List<PathPoint> _createRoadsFromCheckpoints(List<GameLevelCheckPoint> checkpoints) {
    ...
    for (var checkpoint in checkpoints) {
      ...
      var newPointBefore = actual.clone().addVectorToThis(new Vector.fromAngleRadians(angle + pi, checkpoint.lengthBefore));
  ...
  List<PathPoint> _cutCollidingRoads(List<PathPoint> path, bool circular) {
    ...
    if (circular) _cutCollidingRoad(path[path.length - 2], path[path.length - 1], path[0], path[1], data);
    return data;
  }
  ...
  void _fixAngles(List<PathPoint> path, bool circular) {
    for (int i = 1; i < path.length - 1; i++) {
      path[i].angle = (path[i - 1].vector.angleWithThis(path[i + 1].vector)) - pi / 2;
    }
    if (circular) {
      path[0].angle = (path[path.length - 1].vector.angleWithThis(path[1].vector)) - pi / 2;
      path[path.length - 1].angle = (path[path.length - 2].vector.angleWithThis(path[0].vector)) - pi / 2;
    } else {
      path[0].angle = (path[0].vector.angleWithThis(path[1].vector)) - pi / 2;
      path[path.length - 1].angle = (path[path.length - 1].vector.angleWithThis(path[0].vector)) - pi / 2;
    }
  }
}
